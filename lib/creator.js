...
Torrent.prototype = mix({ // Defaults
    ...
    updateFromFile: function (file, callback, listener) {
        ...
        let remaining = this.info['piece length'], processed = 0, available, count, errors = 0, waitingForDigestion = false;
        let onInputStreamReady = () => {
            ...
            try {
                ...
                if (!remaining) {
                    ...
                    waitingForDigestion = false;
                }
                else {
                    waitingForDigestion = true;
                }
                console.log('Remaining: ', remaining, ', Processed: ', processed, ' of ', file.fileSize);
                if (processed === file.fileSize) {
                    if (waitingForDigestion)
                        console.log('Call the finish method if you\'re done updating the torrent.');
                     if (callable)
                         callback(this);
                     return void(0);
                }
    ...
    finish: function () {
        if (Math.ceil(this.info.length/this.info['piece length']) !== (this.info.pieces.length/20)) {
            console.log('Check: ', Math.ceil(this.info.length/this.info['piece length']), (this.info.pieces.length/20))
            this.info.pieces += hasher(this).digest();
        }
