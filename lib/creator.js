...
const { accessor, merge, definePrivateProperty, enumToArray, prefs } = require('./utils');
...
// Returns the default template or the user's preference
const getPrototype = () => {
    try {
        let prototype, sObj = prefs.get('prototype');
        if (sObj)
            prototype = JSON.parse(sObj);
        else {
            sObj = JSON.stringify(prototype = getTemplate());
            prefs.set('prototype', sObj);
        }
        return prototype;
    }
    catch (error) {
...
function Torrent(prototype = {}) {
    ...
    merge(this, getPrototype(), prototype);
}
...
definePrivateProperty(Torrent.prototype, 'title');
...
// Returns a boolean that determines whether or not the URL refers to a local directory.
...
const createTorrent = (options = {}, prototype) => {
    ...
    try {
        ...
        if (!isStream) {
            ...
            // Set the title
            torrent.title = torrent.title || torrent.info.name;
