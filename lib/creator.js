...
const nsIXMLHttpRequest = CC('@mozilla.org/xmlextras/xmlhttprequest;1', 'nsIXMLHttpRequest'); // https://developer.mozilla.org/en-US/docs/nsIXMLHttpRequest
...
Torrent.prototype = obscure({ // Ensure that the following methods can't be enumerated.
    updateFrom: function (source, listener) {
        ...
        try {
            ...
            channel.notificationCallbacks = new nsIProgressEventSink(listener);
...
const createTorrent = (options = {}, prototype) => {
    ...
    try {
        ...
        let { source, torrent, prototype } = options;
        ...
        if (!isStream) {
            ...
            if (!isLocalFile) {
                ...
                else if (typeof torrent['url-list'].push === 'function' &&
                         typeof torrent['url-list'].indexOf === 'function') {
        ...
        // Configure listeners.
        let resolve, reject, { onprogress, onload, onerror } = options;
        if (typeof onload === 'function') {
            resolve = torrent => {
                onload(torrent);
                deferred.resolve(torrrent);
            };
        }
        ...
        // Hash the resource(s)
        if (!multiFileMode)
            torrent.updateFrom(uri || source, onprogress).then(resolve, reject);
        else {
            ...
            if (torrent.info.files && typeof torrent.info.files.push === 'function')
            ...
            let iterate = () => {
                if (files.length)
                    torrent.updateFrom(files.shift(), onprogress).then(iterate, reject);
...
const filePicker = (title = '', modeGetFolder) => {
    let deferred = defer();
    try {
        // File Picker
        ...
        picker.open(result => {
            if (result === picker.returnOK || result === picker.returnReplace) {
                console.log(picker.fileURL);
                deferred.resolve(picker.fileURL);
            }
